...
public class LocalFileCreate extends FileCreateTools{
	...
	public boolean createDir(String Path, String DirName, int posixPermissions)
		...
		if(Dir.exists())
			throw new FileOperateException("4","创建" + DirS + "失败，目标文件已经存在！");
	...
	public boolean createFile(String Path, String FileName, String FileType)
		...
		if(file.exists())
			throw new FileOperateException("4","创建" + fileS + "失败，目标文件已经存在！");
		
		if(fileS.endsWith(File.separator))
			throw new FileOperateException("5","创建" + fileS + "失败，目标文件不能为目录！");
		
		if(!file.getParentFile().exists())
			throw new FileOperateException("6","创建" + fileS + "失败，目标文件所处目录不存在！");
		
		try {
			if(file.createNewFile()) {
				return true;
			}else {
				throw new FileOperateException("7","创建" + fileS + "失败");
			}

		}catch (IOException e) {
			// TODO: handle exception
			throw new FileOperateException("8",e.getMessage());
		}
	}
